...
// SPIバスの設定
      cfg.spi_host = (spi_host_device_t) 2;     // 使用するSPIを選択  ESP32-S2,C3 : SPI2_HOST or SPI3_HOST / ESP32 : VSPI_HOST or HSPI_HOST
...
//*
    { // タッチスクリーン制御の設定を行います。（必要なければ削除）
      ...
      cfg.x_min      = 214;    // タッチスクリーンから得られる最小のX値(生の値)
      cfg.x_max      = 3592;  // タッチスクリーンから得られる最大のX値(生の値)
      cfg.y_min      = 246;    // タッチスクリーンから得られる最小のY値(生の値)
      cfg.y_max      = 3544;  // タッチスクリーンから得られる最大のY値(生の値)
      cfg.pin_int    = 40;   // INTが接続されているピン番号
      cfg.bus_shared = true; // 画面と共通のバスを使用している場合 trueを設定
      cfg.offset_rotation = 0;// 表示とタッチの向きのが一致しない場合の調整 0~7の値で設定

// SPI接続の場合
      cfg.spi_host = (spi_host_device_t) 2;// 使用するSPIを選択 (HSPI_HOST or VSPI_HOST)
